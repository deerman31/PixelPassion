...
func UpdateFullNameHandler(db *sql.DB) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// lastname,firstnameをupdateする
		if _, err := tx.Exec(updateFullNameQuery, req.Lastname, req.Firstname, userID); err != nil {
		...
		return c.JSON(http.StatusOK, map[string]string{"message": "Update FullName successfully."})
	}
}
