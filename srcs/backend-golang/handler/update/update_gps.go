...
func UpdateGpsHandler(db *sql.DB) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// is_gpsをupdateする
		if _, err := tx.Exec(updateGpsQuery, req.IsGpsEnabled, userID); err != nil {
		...
		return c.JSON(http.StatusOK, map[string]string{"message": "Update is_gps successfully."})
	}
}
