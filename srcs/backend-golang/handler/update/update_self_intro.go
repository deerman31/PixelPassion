...
func UpdateSelfIntroHandler(db *sql.DB) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		req := new(UpdateSelfIntroRequest)
		...
		// self_introをupdateする
		if _, err := tx.Exec(updateSelfIntroQuery, req.SelfIntro, userID); err != nil {
		...
		return c.JSON(http.StatusOK, map[string]string{"message": "Update gender successfully."})
	}
}
